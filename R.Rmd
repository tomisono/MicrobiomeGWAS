...
```{} 
setwd('/home/tomi/Desktop/Arena/GENUS_RMD/MR')

# Extract allel frequency
afreq <- read.table('plink2.afreq', header =  TRUE)
afreq <- subset(afreq, select = c("ID","ALT_FREQS"))   # Remember to remove comment in file

# Exposure
exposure <- read.table('Roseburia.phe.phenotype.glm.linear', header =  TRUE)  # Remember to remove comment in file
# Remove covariate results
exposure <- subset(exposure, TEST %in% c("ADD"))

# MERGE allele frequency
exposure = merge(exposure,afreq,by='ID',all = FALSE)

# Filter data frame by significant P values
exposure <- filter(exposure, P < 1e-01 )

colnames(exposure) <- c('SNP','chr_name','chr_start','other_allele.exposure','effect_allele.exposure','A1','TEST',
                        'OBS_CT','beta.exposure','se.exposure','T_STAT','pval.exposure','ERRCODE','eaf.exposure')
...
exposure <- subset(exposure, select = c('SNP','effect_allele.exposure','beta.exposure',
                                        'se.exposure','other_allele.exposure','pval.exposure','eaf.exposure'))
...
```{}
setwd('/home/tomi/Desktop/Arena/GENUS_RMD/MR')
#gunzip("coloncancer.bgz", "coloncancer.tsv")
outcome <- read_tsv("coloncancer.tsv")
outcome <- subset(outcome, select = c("chr", "pos", "alt","beta_EUR","se_EUR","ref","pval_EUR"))
colnames(outcome) <- c("chr_name", "chr_start", 'effect_allele.outcome','beta.outcome','se.outcome','other_allele.outcome','pval.outcome')
...
outcome <- subset(outcome, select = c('SNP','effect_allele.outcome','beta.outcome','se.outcome','other_allele.outcome','pval.outcome'))

outcome$id.outcome <- 'C'
outcome$outcome <- 'Cancer'
...
'mr_ivw', 'mr_simple_mode', 'mr_weighted_median'  ,'mr_weighted_mode'))
...
Forest plot
Here, the plot shows the causal effect as estimated using each of the SNPs on their own.
```{}
Singlesnp_results <- mr_singlesnp(dat,parameters = default_parameters(),all_method = c("mr_ivw", "mr_egger_regression"))
...
# Exposure
exposure <- read.table('Zotu72.phe.phenotype.glm.linear', header =  TRUE)  # Remember to remove comment in file
# Remove covariate results
exposure <- subset(exposure, TEST %in% c("ADD"))
...
# Filter data frame by significant P values
exposure <- filter(exposure, P < 5e-06 )
...
# Select columns of exposure
exposure <- subset(exposure, select = c('SNP','effect_allele.exposure','beta.exposure',
                                        'se.exposure','other_allele.exposure','pval.exposure'))
...
```{}
setwd('/home/tomi/Desktop/Arena/GENUS_RMD/MR')
#gunzip("coloncancer_2.bgz", "coloncancer_2.tsv")
outcome <- read_tsv("coloncancer_2.tsv")
print(colnames(outcome))
##outcome <- subset(outcome, select = c("chr", "pos", "alt","beta_EUR","se_EUR","ref","neglog10_pval_EUR")) 
outcome <- subset(outcome, select = c("chr", "pos", "alt","beta_meta","se_meta","ref","pval_meta")) 
colnames(outcome) <- c("chr_name", "chr_start", 'effect_allele.outcome','beta.outcome','se.outcome','other_allele.outcome','pval.outcome')

#outcome$pval.outcome <- exp(outcome$pval.outcome)
